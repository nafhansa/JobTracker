...
BEGIN
  ...
  purchased_coins := v_purchased + p_amount;
  total_coins := v_weekly + v_purchased + p_amount;
...
BEGIN
  ...
  SELECT EXISTS (
    ...
    WHERE reference_id = p_order_id AND type = 'purchase'
  ...
  SELECT status INTO v_cp_status
  FROM coin_purchases
  ...
  RETURN jsonb_build_object(
    ...
    'new_balance', v_balance.total_coins
  );
END;
$$ LANGUAGE plpgsql;
